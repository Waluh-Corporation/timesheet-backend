...
- **Real-Time Job Notifications & Status APIs**: Users receive a Web Push notification and an email containing a direct secure download button as soon as their spreadsheet is ready. Added `GET /api/v1/timesheet/jobs/:id` and `GET /api/v1/timesheet/jobs` endpoints to check job progress and retrieve download links.
