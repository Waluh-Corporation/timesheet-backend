...
func TestSetupHandlers(t *testing.T) {
	...
	t.Run("InitSetup rejects weak password when uninitialized", func(t *testing.T) {
		...
		if w.Code != http.StatusBadRequest {
			t.Errorf("expected status 400 for weak password, got %d (body: %s)", w.Code, w.Body.String())
		}
	})

	t.Run("InitSetup successfully initializes system with admin and approvers", func(t *testing.T) {
		db, cfg := setupTestDB(t)
		tx := db.Begin()
		defer tx.Rollback()
		_ = tx.Exec("DELETE FROM users WHERE role = 'admin'").Error
		...
		if w.Code != http.StatusOK {
			t.Fatalf("expected status 200 for valid init setup, got %d (body: %s)", w.Code, w.Body.String())
		}
	...
	t.Run("GetSetupStatus reflects admin existence", func(t *testing.T) {
		db, cfg := setupTestDB(t)
		authSvc := auth.NewService("test-secret-at-least-32-chars-long!", cfg.JWTExpiry)
		srv := &Server{
			DB:   db,
	...
	t.Run("InitSetup blocked if admin already exists", func(t *testing.T) {
		db, cfg := setupTestDB(t)
		var adminCount int64
		_ = db.Model(&models.User{}).Where("role = ? AND deleted_at IS NULL", models.RoleAdmin).Count(&adminCount).Error
		if adminCount == 0 {
		...
		if w.Code != http.StatusForbidden {
			t.Errorf("expected status 403 Forbidden when admin exists, got %d", w.Code)
		}
	})

	t.Run("SeedActivityStatuses ensures default statuses", func(t *testing.T) {
		db, _ := setupTestDB(t)
